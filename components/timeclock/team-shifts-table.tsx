...
export default function TeamShiftsTable({ shifts }: { shifts: (ShiftWithComparison & { userName: string })[] }) {
  if (shifts.length === 0) {
    return (
        ...
        <tbody>
          {shifts.map((s, i) => (
            <tr key={s.id} className={i % 2 === 1 ? "bg-card/40" : ""}>
              <td className="px-5 py-3 border-t border-line font-medium">{s.userName}</td>
